...
const Form = ({ formType, formButton, setUser }) => {
  ...
  return (
    <form className="w-full" onSubmit={submitForm}>
      ...
      <button
        className="focus:outline-none cursor-pointer rounded bg-black w-full p-3 text-white font-bold mt-5"
        type="submit"
        disabled={isDisabled}
      >
        {formButton}
      </button>
      {error && <p className="mt-1 text-sm text-black-500">{error}</p>}
    </form>
  );
};
